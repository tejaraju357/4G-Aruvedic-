...
export default function Home() {
  ...
  return (
    <div>
      {/* ── Hero ───────────────────────────────────── */}
      <section style={{ minHeight: '88vh', display: 'grid', gridTemplateColumns: '1fr 1fr', maxWidth: 1400, margin: '0 auto', padding: '0 32px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: 48, paddingTop: 40 }}>
          <div style={{ fontSize: 11, letterSpacing: '.24em', textTransform: 'uppercase', color: 'var(--accent)' }}>{hero.eyebrow}</div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(52px, 6vw, 96px)', fontWeight: 500, lineHeight: 1.0, letterSpacing: '-.025em', margin: '18px 0 0' }}>{hero.title}</h1>
        ...
        <div style={{ display: 'grid', placeItems: 'center', paddingTop: 40, paddingBottom: 40 }}>
          {featuredProduct && (
            <div style={{ width: '100%', maxWidth: 460, cursor: 'pointer' }} onClick={() => navigate(`/product/${featuredProduct.id}`)}>
              <ProductVisual product={featuredProduct} height={500} />
      ...
      {/* ── Category strip ───────────────────────── */}
      <section style={{ background: 'var(--surface)', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)', padding: '28px 32px' }}>
        <div style={{ display: 'flex', gap: 20, maxWidth: 1280, margin: '0 auto', overflowX: 'auto' }}>
          {CATEGORIES.map(cat => (
      ...
      {/* ── Editorial band ───────────────────────── */}
      <section style={{ background: 'var(--soft)', padding: '80px 32px' }}>
      ...
      {/* ── CTA banner ───────────────────────────── */}
      <section style={{ background: 'var(--ink)', color: 'var(--bg)', padding: '64px 32px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <div style={{ fontSize: 11, letterSpacing: '.24em', textTransform: 'uppercase', opacity: .6 }}>Members only</div>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 48, fontWeight: 500, lineHeight: 1.1, marginTop: 16 }}>10% off your first order.</h2>
